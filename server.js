...
async function handleGet(req, res, pathname) {
  ...
  if (pathname.startsWith("/api/admin/")) {
    ...
    if (pathname === "/api/admin/bootstrap") {
      ...
      const [products, orders, customers, subscribers, orderItems, revenue, staffUsers, channelRevenue] = await Promise.all([
...
async function handlePostPutDelete(req, res, method, pathname) {
  ...
  if (orderItemMatch && method === "PUT") {
    ...
    return withClient(async (client) => {
      await client.query("BEGIN");
      try {
        const itemResult = await client.query("SELECT oi.*, o.source AS order_source, o.status AS order_status FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE oi.id = $1", [itemId]);
